use Test::Nginx::Socket::Lua 'no_plan';

our $HttpConfig = <<'_EOC_';
    lua_package_path 'lib/?.lua;;';
_EOC_

no_shuffle();
no_long_string();
run_tests();

__DATA__

=== TEST 1: RSA-OAEP-384 + A256GCM ok
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
            local jwt = require "resty.jwt-verification"
            local token = "eyJhbGciOiJSU0EtT0FFUC0zODQiLCJlbmMiOiJBMjU2R0NNIn0.HZ8fWxDBEVL5NXNqXTqzy0t_2DtUKaLwG13CRACTvdxOsesjygX_yuZLNPqku-Q3HB3B3RypbjMMZqtWGw0oGC_fJ6PlMwI70fE7O98SB_UZs4-gnno0y1HTfEHjbB5xJngQPTe51APW-gmv-o1ReF9_ZWi26t7krBIllgFFb85sqi3R0NAPIUKo_3XSP3jQo6SYTIajZe1Z2FNwC4O6h5nkFNOJu2m3XrfEiZA6XJuiLeySO4HzmkIGfXhYGwSmeXWMDEtxFHYAMYjNi9CVbyTkxPlqei6wO-L0Pfw0uf0fJrZnvtBg2AUXvdIUx-qv5wwnVjDLWDogrth6UGXEZw.eTr9KwNHQXwKdBvO._qfNqcZPORlT6xIxDJ6-n8n15oWVtZC_Gcnge8Ct.Ta009nM6ezw3SUJIrOmSww"
            local decoded_token, err = jwt.decrypt(token, "-----BEGIN PRIVATE KEY-----\\nftaj5h6iEOTu2unqXEMcFsw6\n-----END PRIVATE KEY-----", nil)
            if decoded_token ~= nil then
                ngx.say(decoded_token.header.alg .. "|" .. decoded_token.header.enc)
                ngx.say(decoded_token.payload.foo)
            else
                ngx.say(decoded_token)
                ngx.say(decoded_token)
            end
            ngx.say(err)
        }
    }
--- request
    GET /t
--- response_body
RSA-OAEP-384|A256GCM
bar
nil
--- error_code: 200
--- no_error_log
[error]

=== TEST 2: RSA-OAEP-384 + A256GCM error, wrong secret
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
            local jwt = require "resty.jwt-verification"
            local token = "eyJhbGciOiJSU0EtT0FFUC0zODQiLCJlbmMiOiJBMjU2R0NNIn0.HZ8fWxDBEVL5NXNqXTqzy0t_2DtUKaLwG13CRACTvdxOsesjygX_yuZLNPqku-Q3HB3B3RypbjMMZqtWGw0oGC_fJ6PlMwI70fE7O98SB_UZs4-gnno0y1HTfEHjbB5xJngQPTe51APW-gmv-o1ReF9_ZWi26t7krBIllgFFb85sqi3R0NAPIUKo_3XSP3jQo6SYTIajZe1Z2FNwC4O6h5nkFNOJu2m3XrfEiZA6XJuiLeySO4HzmkIGfXhYGwSmeXWMDEtxFHYAMYjNi9CVbyTkxPlqei6wO-L0Pfw0uf0fJrZnvtBg2AUXvdIUx-qv5wwnVjDLWDogrth6UGXEZw.eTr9KwNHQXwKdBvO._qfNqcZPORlT6xIxDJ6-n8n15oWVtZC_Gcnge8Ct.Ta009nM6ezw3SUJIrOmSww"
            local decoded_token, err = jwt.decrypt(token, "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----", nil)
            if decoded_token ~= nil then
                ngx.say(decoded_token.header.alg .. "|" .. decoded_token.header.enc)
                ngx.say(decoded_token.payload.foo)
            else
                ngx.say(decoded_token)
                ngx.say(decoded_token)
            end
            ngx.say(err)
        }
    }
--- request
    GET /t
--- response_body
nil
nil
invalid jwt: pkey:asymmetric_routine EVP_PKEY_decrypt: code: 0 error:30000E8:digital envelope routines:EVP_PKEY_decrypt:provider asym cipher failure:crypto/evp/asymcipher.c:314:
--- error_code: 200
--- no_error_log
[error]
