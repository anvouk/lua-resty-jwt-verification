use Test::Nginx::Socket::Lua 'no_plan';

our $HttpConfig = <<'_EOC_';
    lua_package_path 'lib/?.lua;;';
_EOC_

no_shuffle();
no_long_string();
run_tests();

__DATA__

=== TEST 1: RSA-OAEP + A128GCM ok
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
            local jwt = require "resty.jwt-verification"
            local token = "eyJhbGciOiJSU0EtT0FFUCIsImVuYyI6IkExMjhHQ00ifQ.AIfpGdvn3MozzOSRh8QSiKB9Ww2MDIvE6BzPcABjBHYI2hb-P2aCIcIMJg8dSxxCOmxoNVB83lZicRddv1OcqP7pcvKm2XdRC0wogT0iUxN11D97ZrHVB0tFA1s1oxmfB4W8smdaS68FVWrtCxMW11ivMW4GCv-LUmqmJdTor7vsAaXvFsUMv2J_sJCJvSLH0w4vY6XEoLnogQsugY2RC8c3rB-pR5gEYWtGRCMcsTEe7R9F_x8IlXvoA_OOtVUgNvsmsjG1OhcX3ofNrSCAMAjWYjChs5OH0PIZaaurm3NYnFkt0F7PqJkVIlRDXoIw-k2R1nS6D5xLnER-7N1i1w.zwgDOgLmTcgQJ0fH.uXF_WaE9XhEOcU3bNc6ExJaGeBTLT_8sU53sBS50.uhjboACPF0dp3pDBnTYryQ"
            local decoded_token, err = jwt.decrypt(token, "-----BEGIN PRIVATE KEY-----\\nftaj5h6iEOTu2unqXEMcFsw6\n-----END PRIVATE KEY-----", nil)
            if decoded_token ~= nil then
                ngx.say(decoded_token.header.alg .. "|" .. decoded_token.header.enc)
                ngx.say(decoded_token.payload.foo)
            else
                ngx.say(decoded_token)
                ngx.say(decoded_token)
            end
            ngx.say(err)
        }
    }
--- request
    GET /t
--- response_body
RSA-OAEP|A128GCM
bar
nil
--- error_code: 200
--- no_error_log
[error]

=== TEST 2: RSA-OAEP + A128GCM error, wrong secret
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
            local jwt = require "resty.jwt-verification"
            local token = "eyJhbGciOiJSU0EtT0FFUCIsImVuYyI6IkExMjhHQ00ifQ.AIfpGdvn3MozzOSRh8QSiKB9Ww2MDIvE6BzPcABjBHYI2hb-P2aCIcIMJg8dSxxCOmxoNVB83lZicRddv1OcqP7pcvKm2XdRC0wogT0iUxN11D97ZrHVB0tFA1s1oxmfB4W8smdaS68FVWrtCxMW11ivMW4GCv-LUmqmJdTor7vsAaXvFsUMv2J_sJCJvSLH0w4vY6XEoLnogQsugY2RC8c3rB-pR5gEYWtGRCMcsTEe7R9F_x8IlXvoA_OOtVUgNvsmsjG1OhcX3ofNrSCAMAjWYjChs5OH0PIZaaurm3NYnFkt0F7PqJkVIlRDXoIw-k2R1nS6D5xLnER-7N1i1w.zwgDOgLmTcgQJ0fH.uXF_WaE9XhEOcU3bNc6ExJaGeBTLT_8sU53sBS50.uhjboACPF0dp3pDBnTYryQ"
            local decoded_token, err = jwt.decrypt(token, "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----", nil)
            if decoded_token ~= nil then
                ngx.say(decoded_token.header.alg .. "|" .. decoded_token.header.enc)
                ngx.say(decoded_token.payload.foo)
            else
                ngx.say(decoded_token)
                ngx.say(decoded_token)
            end
            ngx.say(err)
        }
    }
--- request
    GET /t
--- response_body_like
nil
nil
invalid jwt: pkey:asymmetric_routine EVP_PKEY_decrypt: code: 0 error:2000079:rsa routines:RSA_padding_check_PKCS1_OAEP_mgf1:oaep decoding.+
--- error_code: 200
--- no_error_log
[error]
